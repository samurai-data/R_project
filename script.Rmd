...
#triste
```{r}
wtd.table(coco1$triste,coco1$sexe,
          weights = coco1$coco1_POIDS, useNA = "ifany") %>% 
  rprop(digits=1)

wtd.table(coco6$triste,coco6$sexe,
          weights = coco6$POIDS_coco6, useNA = "ifany") %>% 
  rprop(digits=1)
```
#### graphique : triste
...
```{r}
#fusionner les deux vagues
s_triste <- cbind(triste6, triste6)
#presenter le tableau
...
coco1$sommeil <- fct_temps_actvt(coco1$coco1_q59_1)
...
#remarque: lors de la 6ième vague les français dorment beaucoup plus (70% de la population passent 6h et plus à dormir alors que lors de la 1 vague 84 % consacre 4h ou moins pour le sommeil dont une partie ne dort pas du tout)
```
...
```{r}
#eayy_B1B: Exercice d'une activité professionnelle
#Variable recodée à partir de la variable B1 (Quelle est votre situation #principale vis-à-vis du travail ?) et B1A (Occupez-vous également un #emploi ?; universe=(B1 in 3:8)). Si l'interviewé a déclaré avoir un #emploi tout en étant inactif, chômeur, retraité alors le recodage B1B #est oui

fct_activ_prof <- function(x){
  recod <- fct_collapse(factor(x),
  "Oui"="1", 
  "Non"="2", 
  "7"="7")
...
coco1$activ_prof <- fct_activ_prof(coco1$eayy_B1B)
coco6$activ_prof <- fct_activ_prof(coco6$eayy_B1B)

wtd.table(coco1$sexe,coco1$activ_prof,
          weights = coco1$coco1_POIDS, useNA = "ifany", exclude = '7') %>% 
  prop(digits=1)

wtd.table(coco6$sexe,coco6$activ_prof,
          weights = coco6$POIDS_coco6, useNA = "ifany", exclude = '7') %>% 
  prop(digits=1)

#remarque: pas de diff signif
```


### Conditions de vie et les activités de la vie quotidienne
