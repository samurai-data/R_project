...
reg_sf <- st_read(dsn="regions-20180101.shp")
class(reg_sf)
names(reg_sf)
str(reg_sf)
```
...
anx <- read.csv2("donnees_anxiete_reg_vague1.csv") # import
names(anx)
str(anx)
...
# Carte taux de l'anxiété par région mais ça ne marche pas
ggplot()+
  geom_sf(data=reg_sff,aes(fill="anxiete"),color="snow4",size=.2)+
  viridis::scale_fill_viridis()+
  theme_bw()+
  labs(title = "Le taux d'état anxieux par region durant la première vague",  
       fill="anxiete_groupe", caption="Source : Données d’enquête COVIPREV")+
  coord_sf( datum = NA)
