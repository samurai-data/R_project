...
Les taux de cohabitation des couples sont ensuite comparés.
La cohabitation au sein des couples est presque également répartie entre les hommes et les femmes. Et tant pour les hommes que pour les femmes, le nombre de couples non cohabitants était d'environ 3,5% pour les hommes et 4,8% pour les femmes lors de la vague 1. Cela n'a pas beaucoup changé lors de la vague 6.
Les résultats montrent qu'il est fréquent que les couples vivent ensemble. Et le confinement n'a pas été un facteur de promotion de la cohabitation.
...
vg1_genre_solitude %>% 
  ...
  labs(title="Solitude selon le genre pendant la vague 1",
       y="%",fill="",
...
vg6_genre_solitude %>% 
  ...
  aes(fill=sexe, x=solitude, y= Freq) %>% 
  geom_bar(col="black",stat="identity", position="dodge")+
  labs(title="Solitude selon le genre pendant la vague 6",
       y="%",fill="",
...
#fusionner les deux vagues
s_decourage <- cbind(decourage1, decourage6)
#presenter le tableau
...
Dans les vagues précédentes de solitude, de nervosité et de décourage, les femmes avaient toujours ces sentiments ("Souvent") dans la première vague, mais dans la sixième vague, on a cnstaté que les femmes avaient tendance à avoir ces sentiments plus fortement ou elles ne changent rien. Cela signifie qu'il y a une raison pour que les femmes éprouvent plus fortement ces sentiments négatifs.
...
La grande majorité de la population étudiée a repondu de ne jamais voir (rencontres physiques) leurs amis ou les membres de leur famille en dehors des personnes qui sont logées avec elles deux semaines precedant l'enquete. Les resultats sont similaires pour les deux vagues et les deux sexes. 
...
On remarque que le nombre des femmes qui se sentent en premanence, souvent ou quelques fois nerveux pendant les deux semaines précedant l'enquête est significativement plus elévé. Ainsi, cela serait judicieux de tester cette hypothèse avec le test khi-2. 
...
```{r, results='hide'}
# Supprimer les régions dom-tom d'abord
reg_sf$code_insee
reg_sf2 <- reg_sf %>% slice(-c(1, 2, 3, 8, 12, 14))
ggplot()+
  geom_sf(data=reg_sf2)
# Transformer
reg_sf2 <- st_transform(reg_sf2,crs=2154)
```
**Ajout de données** 

Depuis le 23 mars 2020, Santé publique France a lancé l'enquête CoviPrev en population générale afin de suivre l’évolution des comportements (gestes barrières, confinement, consommation d’alcool et de tabac, alimentation et activité physique) et de la santé mentale (bien-être, troubles) de la population en France métropolitaine. On va voir la carte du taux de la population ayant l'anxiété durant la vague 1
...
#Fusionner
reg_sff <- left_join(reg_sf2, anx, by =c("code_insee" = "reg"))
```

**Carte de l'anxiété selon region**
```{r}
ggplot()+
  geom_sf(data=reg_sff,aes(fill=anxiete),color="snow4",size=.2)+
  viridis::scale_fill_viridis()+
  theme_bw()+
  labs(title = "Le taux de population avec l'anxiété au premier vague de covid",  
       fill="%", caption="Source : L'enquête CoviPrev 2020")+
  coord_sf( datum = NA)
```
**Commentaire**

Au cours du première vague en fin mars 2020, les regions où la population a le plus haut taux d'anxiété est Bourgogne-Franche-Comté, puis ce sont Île-de-France, Grand-Est, Hauts-de-France. On peut expliquer cela par la réalité que ce sont aussi les regions où le taux de positivité des tests pour le SARS-CoV-2 au 24 mars 2020 sont le plus haut. Ayant les proches testés positive peut ramener les sentimens négative tel que l'anxiété.

# Conclusion 

Les émotions négatives ressenties lors du confinement étaient plus fortement ressenties par les femmes. On suppose donc qu'il y a un certain éléments évoquant ce phénomène. 

Nous avons donc examiné la santé physique, la vie sociale et l'avis sur le confinement des hommes et des femmes durant les deux vague du covid : le vague 1 et le vague 6. 

Concernant la santé physique, l'obésité était plus répandue chez les hommes alors que les femmes étaient plus susceptibles d'avoi un poids correct. Les hommes sont plus nombreux à pratiquer le sport tous les jours que les femmes mais les femmes semblent plus nombreuses à dormir plus de 8h par jour. On peut imaginer que les hommes ont plus de temps libres à faire certaines activités recréatives comme le sport. En revanche, les femmes ont plutôt besoin de se reposer après avoir fait les tâches toute la journée.

Concernant la vie sociale, les resultats sont similaires pour les deux vagues et les deux sexes : La majorité de la population étudiée a repondu de ne jamais voir (rencontres physiques) leurs amis ou les membres de leur famille.

En regardant à l'organisation des tâches ménagères, on voit que les femmes sont plus nombreuses à consacrer plus de deux heures par jour à faire du menage alors que qu'environ 60 % des hommes consacrent moins d'une heure à s'occuper du ménage. Cela suggère que le temps consacré aux tâches ménagères peut être un facteur contribuant aux sentiments négatifs chez les femmes.

Les femmes et les hommes sont affecté par le confinement presque de la même manière car leur niveau de stresse se rassemble. Pourtant, on remarque quelques différences dans le vécu du confinement et du télétravail. Les femmes sont moins nombreuses que les hommes à penser que leur vie quotidienne en confinement leur permet à profiter de la vie et le télétravail leur permet de se concentrer. 

Nous pouvons conclure que les femmes et les hommes ont les manières différentes d'organiser leur activités durant le temps des vagues du covid. Le fait de devoir concilier les tâches ménagères, le travail et la vie sociale est probablement la raison pourlaquelle les femmes sont plus nombreuses à se sentir nerveuse que les hommes. 