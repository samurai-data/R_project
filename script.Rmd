...
fct_temps_enf <- function(x){
  ...
  "Entre 2 et 3 heures"="4",
  "Entre 3 et 4 heures"= "5",
  "6 heures et plus" = c("6","7", "8", "9"),
  "PDR" = "9999")
  return(recod)
}


coco1$enf <- fct_temps_actvt(coco1$coco1_q59_2)
coco6$enf <- fct_temps_actvt(coco6$coco6_q59_2)

#vague 1
enf_1 <- wtd.table(coco1$enf, coco1$sexe, weights = coco1$coco1_POIDS, exclude = c("PDR", "9999"),useNA = "ifany") %>%
  prop(digits=1)

#vague 6
enf_6 <- wtd.table(coco6$enf, coco6$sexe, weights = coco6$POIDS_coco6, exclude = c("PDR", "9999"),useNA = "ifany") %>%
  prop(digits=1)

#remarque: consacrent plus de temps aux enfants après le confinement
```
...
```{r}
#fusionner les deux vagues
t_enf_df <- cbind(enf_1, enf_6)

#présenter le tableau

t_enf_df <- as.data.frame(t_enf_df)
colnames(t_enf_df) <- c("Homme", "Femme", "Total", "homme", "femme", "total")

t_enf_df <- rownames_to_column(t_enf_df, "Fréquence")
...
fct_temps_soin <- function(x){
  recod <- fct_collapse(factor(x),
  "Je n’ai pas fait cette activité" = "1",
  "Entre 0 et 1 heure"= "2", 
  "Entre 1 et 2 heures"="3",
  "Entre 2 et 3 heures"="4",
  "3 heures et plus" = c("5", "6","7", "8", "9"),
  "PDR" = "9999")
  return(recod)
}


coco1$soin <- fct_temps_soin(coco1$coco1_q59_3)
coco6$soin <- fct_temps_soin(coco6$coco6_q59_3)

#vague 1
soin_1 <- wtd.table(coco1$soin, coco1$sexe, weights = coco1$coco1_POIDS, exclude = c("PDR","9999"),,useNA = "ifany") %>%
  prop(digits=1)

#vague 6
soin_6 <- wtd.table(coco6$soin, coco6$sexe, weights = coco6$POIDS_coco6, exclude = c("PDR","9999"),useNA = "ifany") %>%
  prop(digits=1)
...
```{r}
#fusionner les deux vagues
t_soin_df <- cbind(soin_1, soin_6)

#présenter le tableau

t_soin_df <- as.data.frame(t_soin_df)
colnames(t_soin_df) <- c("Homme", "Femme", "Total", "homme", "femme", "total")

t_soin_df <- rownames_to_column(t_soin_df, "Fréquence")
